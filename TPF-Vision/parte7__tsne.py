...
def main():
    ...
    tsne_file  = os.path.join(script_dir, 'tsne_tuned_50.png')
    ...
    pca = PCA(n_components=min(5, feats_scaled.shape[0]-1), random_state=42)
    feats_reduced = pca.fit_transform(feats_scaled)

    n_sel     = feats_reduced.shape[0]
    perp      = max(5, min(50, n_sel // 3))
    tsne = TSNE(
        n_components=2,
        perplexity=perp,
        learning_rate=200,
    ...
    embeds = tsne.fit_transform(feats_reduced)
    ...
    plt.title("t-SNE de activaciones")
    plt.xlabel("Dimensión 1")
    plt.ylabel("Dimensión 2")
    plt.tight_layout()
    plt.savefig(tsne_file, dpi=150)
    plt.show()
