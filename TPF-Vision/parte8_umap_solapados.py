...
def main():
    ...
    plt.scatter(df_umap["UMAP1"], df_umap["UMAP2"], s=10, alpha=0.2, color="gray")

    for i, cls in enumerate(class_names):
        df_focus = df_umap[df_umap["Clase"] == cls]

        plt.scatter(df_focus["UMAP1"], df_focus["UMAP2"], s=15, alpha=0.6, label=cls, color=colors[i])
    ...
    plt.title("Zonas de concentración por clase en UMAP")
    plt.xlabel("UMAP1")
    plt.ylabel("UMAP2")
    plt.legend(loc='upper right', bbox_to_anchor=(1.15, 1.0), fontsize="small")
    plt.tight_layout()
    plt.savefig(os.path.join(script_dir, "umap_superpuesto.png"), dpi=150)
    plt.show()
